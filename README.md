...
| 类型              | 说明                 | key可为null | value可为null | 线程安全 |
| ----------------- | -------------------- | ----------- | -------- | -------- |
| HashMap           | 无序                 | 是          | 是       |否       |
| Hashtable         | 无序                 | 否          | 否       |是       |
| TreeMap           | 根据key排序          | 否          | 是       | 否       |
| LinkedHashMap     | 保存了记录的插入顺序 | 是          | 是       |否       |
| ConcurrentHashMap | 无序                 | 否          | 否       |是       |
...
## [高频热点账户(和库存类似但稍有不同)如何解决高并发余额计数问题？](https://blog.csdn.net/fei33423/article/details/47340675) 
...
一但钱被消费了, 账户余额可能少于订单的冻结的金额,可提现余额可能为负数. 
